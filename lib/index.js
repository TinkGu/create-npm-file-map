...
function getMappingPkgInfo(config, moduleName) {
  ...
  function getRelativePath(p) {
    ...
    const dirName = path.join(rootPath, distDir, moduleName, 'index.js');
    const fileName = path.join(rootPath, distDir, moduleName + '.js');
  ...
  function toTypes(p) {
    const distDir = resolveDir(p);
    const dirName = path.join(rootPath, distDir, moduleName + '.d.ts');
    return path.relative(moduleMappingPath, dirName);
  }
